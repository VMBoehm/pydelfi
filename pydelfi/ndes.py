...
class MixtureDensityNetwork:
    ...
    def __init__(self, n_parameters, n_data, n_components = 3, n_hidden=[50,50], activations=[tf.tanh, tf.tanh],
        ...
        sigma_mat  = tfd.matrix_diag_transform(tfd.fill_triangular(self.sigma), transform=tf.exp)
        #sigma_mat  = tfd.matrix_diag_transform(sigma_mat, transform=lambda x: x+1e-38)
        # get Gaussian mixtures from tensorflow
        gmm = tfd.MixtureSameFamily(mixture_distribution=tfd.Categorical(probs=self.alpha),components_distribution=tfd.MultivariateNormalTriL(loc=self.mu,scale_tril=sigma_mat))
        ...
        # Log likelihoods
        self.L     = tf.maximum(gmm.log_prob(self.data),-40)
